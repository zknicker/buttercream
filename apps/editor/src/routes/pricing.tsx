    ...
    <div className="min-h-dvh">
      ...
      <main>
        <section className="mx-auto w-full max-w-6xl px-6 pt-16 pb-14 lg:px-10">
          ...
          <h1 className="mt-5 max-w-[20ch] font-display text-6xl tracking-tight text-balance text-ink lg:text-7xl">
            The menu.
          </h1>
          <p className="mt-6 max-w-[56ch] text-lg text-pretty text-graphite sm:text-base">
        ...
        >
          <h2 className="font-display text-3xl tracking-tight text-ink" id="compare-heading">
            Compare the tiers
          </h2>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-ink/12">
                    <th className="pb-3 font-mono text-sm font-normal whitespace-nowrap text-graphite sm:text-xs">
                      Feature
                    </th>
                    {["Sample", "Bakery", "Patisserie"].map((tier) => (
                      <th
                        className="pb-3 font-mono text-sm font-normal whitespace-nowrap text-graphite sm:text-xs"
                        key={tier}
                      >
                        {tier}
                      </th>
                ...
                <tbody>
                  {COMPARISON.map((row) => (
                    <tr className="border-b border-ink/8" key={row.feature}>
                      <th className="py-3.5 text-base font-normal whitespace-nowrap text-ink sm:text-sm">
                        {row.feature}
                      </th>
        ...
        <section className="mx-auto grid w-full max-w-6xl gap-12 px-6 pb-20 lg:grid-cols-2 lg:gap-16 lg:px-10">
          <div>
            <h2 className="font-display text-3xl tracking-tight text-ink" id="faq-heading">
              Questions
            </h2>
            <Accordion.Root aria-labelledby="faq-heading" className="mt-6">
              {FAQ.map((item) => (
                <Accordion.Item className="border-b border-ink/10" key={item.question}>
                  <Accordion.Header>
                    <Accordion.Trigger className="group flex w-full items-center justify-between gap-4 py-4 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink">
                      <span className="text-base text-ink sm:text-sm">{item.question}</span>
                      <HugeiconsIcon
                        aria-hidden="true"
                        className="size-4 shrink-0 text-graphite transition-transform group-data-panel-open:rotate-45"
                        icon={PlusSignIcon}
                        size={16}
                        strokeWidth={2}
                      />
                  ...
                  <Accordion.Panel className="h-(--accordion-panel-height) overflow-hidden transition-[height] duration-200 data-ending-style:h-0 data-starting-style:h-0">
                    <p className="max-w-[56ch] pb-4 text-base text-pretty text-graphite sm:text-sm">
                      {item.answer}
                    </p>
                  </Accordion.Panel>
                </Accordion.Item>
          ...
          <Surface className="h-fit p-8" variant="secondary">
            <h2 className="font-display text-2xl text-ink">Stay in the loop</h2>
            ...
            >
              <label className="flex flex-col gap-1.5" htmlFor="changelog-email">
                <span className="font-mono text-sm text-graphite uppercase sm:text-xs">Email</span>
                <Input
                  autoComplete="email"
                  fullWidth
                  id="changelog-email"
              ...
              <p className="text-base text-graphite sm:text-sm">No spam. Unsubscribe anytime.</p>
...
function ComparisonCell({ value }: { value: boolean | string }) {
  if (typeof value === "string") {
    return (
      <td className="py-3.5 font-mono text-sm tabular-nums whitespace-nowrap text-graphite sm:text-xs">
        {value}
      </td>
    );
  }
  ...
  return (
    <td className="py-3.5">
      {value ? (
        ...
        <span aria-label="Not included" className="block h-px w-3 bg-ink/25" role="img" />
      )}
    </td>
  );
}
