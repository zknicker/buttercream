...
export function ProductShot({ className }: { className?: string }): ReactElement {
  return (
    <div
      className={classes(
        "overflow-hidden rounded-[min(1vw,12px)] bg-parchment shadow-2xl shadow-ink/12 ring-1 ring-ink/10",
        className,
      )}
    >
      <div className="flex items-center gap-1.5 border-b border-ink/8 bg-crumb px-3 py-2.5">
        ...
        <p className="ml-2 font-mono text-xs text-graphite">buttercream / studio</p>
      </div>
      ...
      <div className="grid grid-cols-1 @2xl:grid-cols-[1fr_13rem]">
        <div className="flex min-w-0 flex-col gap-5 p-6">
          ...
          <div className="flex flex-col gap-1.5">
            <p className="font-mono text-xs text-graphite uppercase">Label</p>
            <div className="flex h-8.5 items-center rounded-(--radius-shell) bg-parchment px-2.5 text-sm text-graphite ring-1 ring-ink/12 ring-inset">
              Pistachio cream
            </div>
          </div>

          <div className="grid gap-3 @md:grid-cols-2">
            <div className="rounded-(--radius-shell) bg-crumb p-4">
              <p className="font-mono text-xs text-graphite uppercase">Orders</p>
              <p className="mt-1 font-display text-2xl tabular-nums text-ink">1,248</p>
              <p className="font-mono text-xs text-crust">+18.6%</p>
            </div>
            <div className="flex items-center justify-between rounded-(--radius-shell) bg-crumb p-4">
              <p className="text-sm text-ink">Active oven</p>
              <span
                aria-hidden="true"
                className="flex h-5 w-9 items-center rounded-full bg-butter p-0.5"
              >
                <span className="ml-auto size-4 rounded-full bg-parchment" />
              </span>
            </div>
          </div>
            ...
            {["Croissant", "Sourdough", "Financier"].map((item, index) => (
              ...
              >
                <div className="flex min-w-0 items-center gap-2.5">
                  <span
                    aria-hidden="true"
                    className="size-6 shrink-0 rounded-full bg-crumb ring-1 ring-ink/8"
                  />
                  <p className="truncate text-sm text-ink">{item}</p>
                </div>
                <p className="shrink-0 font-mono text-xs tabular-nums text-graphite">
                  {[128, 96, 64][index]}
                </p>
              </div>
            ))}
        ...
        <div className="flex flex-col gap-2 border-ink/8 bg-crumb/60 p-4 max-@2xl:border-t @2xl:border-l">
          <div className="flex gap-0.5 rounded-(--radius-shell) bg-ink/6 p-0.5">
            {["Style", "Tokens", "Agent"].map((tab, index) => (
              <span
                className={classes(
                  "flex-1 rounded-[calc(var(--radius-shell)-0.125rem)] py-1 text-center font-mono text-xs",
                  index === 0 ? "bg-parchment text-ink" : "text-graphite",
                )}
                key={tab}
              >
                {tab}
          ...
          {TOKEN_ROWS.map((row) => (
            <div key={row.label}>
              {row.group ? (
                <p className="mt-3 mb-1 font-mono text-xs tracking-wide text-graphite uppercase">
                  {row.group}
                </p>
              ) : null}
              <div className="flex h-8 items-center justify-between gap-2 rounded-(--radius-shell) bg-parchment px-2.5">
          ...
          <div className="mt-3 rounded-(--radius-shell) bg-parchment p-3">
            <Badge variant="accent">Agent</Badge>
            <p className="mt-2 font-mono text-xs leading-5 text-graphite">
